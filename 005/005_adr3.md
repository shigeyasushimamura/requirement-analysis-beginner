# ADR-005: 予約期間と階乗権限の分離によるドメインモデル設計
...
- 「ユーザが予約した時間(課金対称)」と「実際に鍵が開く時間(前後マージンを含む)」にはズレがある
- これらを単一のオブジェクトや DB カラムで管理すると、拡張ロジックが各所に散らばり、変更に弱くなる
...
- Reservation に grantLockCredential()メソッドを持たせて、「5 分前に入室可能にする」といったびじねするーるを、このメソッド内にカプセル化する。
